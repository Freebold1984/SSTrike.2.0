...
from core.template_handler import detect_template_context, analyze_template_context, check_prototype_pollution, get_pollution_payloads
...
def scan(target, paramData, encoding, headers, delay, timeout, skipDOM, skip):
    ...
    template_analysis = analyze_template_context(response)
    if template_analysis['is_template']:
        logger.good('Potential template injection found!')
        logger.info('Template type: %s' % template_analysis['template_type'])
        logger.info('Suggested payloads:')
        for payload in template_analysis['payloads']:
    ...
    for paramName in params.keys():
        ...
        if not occurences:
            ...
            # Check for prototype pollution possibilities in reflections
            if check_prototype_pollution(response):
                logger.good('Potential prototype pollution vector found!')
                logger.info('Suggested prototype pollution payloads:')
                for payload in get_pollution_payloads():
                    logger.info('- %s' % payload)
                logger.red_line()
